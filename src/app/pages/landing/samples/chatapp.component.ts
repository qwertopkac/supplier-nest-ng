        ...
        <div class="w-8/12 xl:w-6/12 border-x border-surface flex flex-col">
            ...
            <div class="flex-1 overflow-y-auto flex flex-col gap-20 py-20 px-12">
                <div *ngFor="let message of chatMessages" class="flex items-start min-w-64 w-fit max-w-[60%]" [ngClass]="{ 'ml-auto mr-0 flex-row-reverse': message.type === 'sent' }">
                    ...
                    <div [ngClass]="message.type === 'received' ? 'flex-1 bg-surface-100 dark:bg-surface-800 px-2 py-1 rounded-lg' : 'flex-1 bg-primary text-primary-contrast px-2 py-1 rounded-lg'">
            ...
            <div class="p-6 border-t border-surface flex items-end justify-between gap-2">
                <div class="flex items-end gap-1 flex-1">
                    ...
                    <textarea pTextarea class="ml-1 flex-1 border-surface-0 dark:border-surface-900 shadow-none max-h-32 min-h-9 bg-emphasis overflow-auto" autoResize rows="1" placeholder="Write your message..."></textarea>
                </div>
