...
export class CategoryService {
	...
	controller: string = 'Categories';

	async getAll(page: number = 0, size: number = 5,SearchTerm:string, successCallBack?: () => void, errorCallBack?: (errorMessage: string) => void): Promise<ApiResponse> {
		const observable: Observable<any> = this.httpCLientService.get({
			controller: this.controller,
			queryString: `PageIndex=${page}&PageSize=${size}&SearchTerm=${SearchTerm}`
		});
